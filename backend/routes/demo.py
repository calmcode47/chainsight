...
async def trigger_demo_alert(authorized: bool = Depends(require_demo_key)):
    ...
    if not shipments:
        raise HTTPException(status_code=404, detail="No on-time shipments found to disrupt")
...
async def simulate_bulk_optimization(authorized: bool = Depends(require_demo_key)):
    ...
    for s in targets:
        ...
        opt_data = {
            ...
            "time_saving_hours": recommendation.get('metrics', {}).get('time_saving_hours'),
            "cost_delta_usd": recommendation.get('metrics', {}).get('cost_delta_usd'),
            "risk_reduction_percent": recommendation.get('metrics', {}).get('risk_reduction_pct'),
