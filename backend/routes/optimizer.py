...
async def optimize_single_route(shipment_id: str, user: dict = Depends(require_auth)):
    ...
    rec = {
        ...
        "alternative_route": recommendation.get("alternative_route")
        or recommendation.get("alternative_waypoints")
        or [],
        ...
        "risk_reduction_percent": int(
            recommendation.get(
                "risk_reduction_percent", recommendation.get("risk_reduction_pct", 0)
            )
        ),
        "recommended_carrier": str(
            recommendation.get("recommended_carrier")
            or recommendation.get("carrier_recommendation", "Maersk")
        ),
        "gemini_reasoning": str(
            recommendation.get("gemini_reasoning")
            or recommendation.get("reasoning")
            or "Optimized route suggested by AI."
        ),
    }
...
async def accept_optimization(
    ...
    if shipment and shipment["status"] == "critical":
        await db.update_shipment_status(
            shipment_id, "at_risk", shipment["delay_hours"] // 2
        )
...
async def batch_optimize_at_risk_shipments(user: dict = Depends(require_auth)):
    ...
    for s in at_risk[:5]:
        ...
        rec = {
            ...
            "risk_reduction_percent": int(
                recommendation.get(
                    ...
                    recommendation.get("risk_reduction_pct", 0),
                )
            ),
            "recommended_carrier": str(
                recommendation.get("recommended_carrier")
        ...
        results.append({"shipment_id": s["id"], "recommendation": rec})
