...
async def get_metrics_history(days: int = Query(7, ge=1, le=30), user: dict = Depends(require_auth)):
    """
    ...
    for i in range(days):
        ...
        if not found:
            ...
            new_snap['snapshot_at'] = datetime.combine(target_date, datetime.min.time()).isoformat() + "Z"
...
async def get_analytics_summary(user: dict = Depends(require_auth)):
    ...
    for s in snapshots:
        on_time_pct = (s['on_time'] / s['total_shipments'] * 100) if s['total_shipments'] > 0 else 0
    ...
    first_pct = (snapshots[0]['on_time'] / snapshots[0]['total_shipments'] * 100) if snapshots[0]['total_shipments'] > 0 else 0
    last_pct = (snapshots[-1]['on_time'] / snapshots[-1]['total_shipments'] * 100) if snapshots[-1]['total_shipments'] > 0 else 0
