...
async def get_all_shipments(
    status: Optional[str] = Query(None, description="Filter by status (on_time, at_risk, delayed, critical)"),
...
async def get_global_metrics(user: dict = Depends(require_auth)):
    ...
    if not latest:
        raise HTTPException(status_code=404, detail="No metrics found. Please seed the database.")
    return latest
