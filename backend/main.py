...
# Configure CORS
origins = [FRONTEND_URL, "http://localhost:5173"] if not IS_PRODUCTION else [FRONTEND_URL]
...
async def schedule_metrics_snapshots():
    ...
    while True:
        try:
            ...
            metrics = mock.get_metrics([Shipment(**s) for s in shipments_data]).model_dump()
...
async def startup_event():
    print(f"🚀 Initializing ChainSight Backend (Env: {'Production' if IS_PRODUCTION else 'Development'})...")
