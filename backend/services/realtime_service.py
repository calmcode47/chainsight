        ...
        while True:
            try:
                ...
                if shipments:
                    ...
                    await self.db.update_shipment_status(
                        target["id"], new_status, new_delay
                    )
                    print(
                        f"✅ [Realtime Sim] Shipment {target['id']} moved to {new_status}"
                    )
        ...
        while True:
            try:
                ...
                at_risk = [
                    s
                    for s in shipments
                    if s["status"] in ["at_risk", "delayed", "critical"]
                ]
                ...
                if at_risk:
                    ...
                    new_alert = {
                        ...
                        "affected_region": target.get("current_location", {}).get(
                            "country"
                        )
                        or "Global",
                    ...
                    print(
                        f"✅ [Realtime Sim] New {new_alert['severity']} alert created for {target['id']}"
                    )
            except Exception as e:
