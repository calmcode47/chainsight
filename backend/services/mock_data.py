...
class MockDataService:
    ...
    def generate_shipments(self, count: int = 50) -> List[Shipment]:
        ...
        for i in range(count):
            ...
            progress = int(
                max(
                    0, min(100, (31 - days_to_eta) / 31 * 100 + random.randint(-10, 10))
                )
            )
            ...
            if progress < 10:
                ...
                current_geo = GeoPoint(
                    lat=origin_geo.lat
                    + (dest_geo.lat - origin_geo.lat) * (progress / 100),
                    lng=origin_geo.lng
                    + (dest_geo.lng - origin_geo.lng) * (progress / 100),
                    city="In Transit",
    ...
    def generate_alerts(self, shipments: List[Shipment]) -> List[DisruptionAlert]:
        ...
        for shp in shipments:
            ...
            severity = "low"
            if shp.status == "critical":
                severity = "critical"
            elif shp.status == "delayed":
                severity = "high"
            elif shp.status == "at_risk":
                severity = "medium"
