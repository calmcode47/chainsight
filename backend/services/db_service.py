...
class DBService:
    def __init__(self):
        ...
        try:
            ...
            print(f"⚠️ Supabase initialization failed: {e}. Falling back to In-Memory Mock mode.")
            self.supabase = None
            self.use_mock = True
    ...
    def _init_mock_data(self):
        ...
        for i in range(7):
            ...
            m["snapshot_at"] = (datetime.utcnow() - timedelta(days=7-i)).isoformat() + "Z"
            self._metrics_history.append(m)
        self._optimizations = []
        print(f"✅ In-memory store ready with {len(self._shipments)} shipments and {len(self._alerts)} alerts.")
    ...
    # SHIPMENTS
    async def get_all_shipments(self, status: str = None, mode: str = None, limit: int = 50) -> List[dict]:
        if self.use_mock:
            data = self._shipments
            if status: data = [s for s in data if s['status'] == status]
            if mode: data = [s for s in data if s['mode'] == mode]
    ...
    async def get_shipment_by_id(self, shipment_id: str) -> dict:
        if self.use_mock:
            for s in self._shipments:
                if s['id'] == shipment_id: return self._map_shipment(s.copy())
            raise HTTPException(status_code=404, detail="Shipment not found")

        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("shipments").select("*").eq("id", shipment_id).single().execute())
            return self._map_shipment(result.data)
        except Exception as e:
            if "matching no rows" in str(e): raise HTTPException(status_code=404, detail="Shipment not found")
            self._handle_error(e)
    ...
    async def upsert_shipments(self, shipments: List[dict]) -> int:
        ...
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("shipments").upsert(shipments, on_conflict="id").execute())
            return len(result.data) if result.data else 0
        except Exception as e:
            self._handle_error(e)

    async def update_shipment_status(self, shipment_id: str, status: str, delay_hours: int = 0) -> bool:
        if self.use_mock:
            for s in self._shipments:
                if s['id'] == shipment_id:
                    s['status'] = status
        ...
        try:
            client = get_supabase()
            data = {"status": status, "delay_hours": delay_hours, "updated_at": now_iso()}
            result = await anyio.to_thread.run_sync(lambda: client.table("shipments").update(data).eq("id", shipment_id).execute())
            return len(result.data) > 0
        except Exception as e:
            self._handle_error(e)

    # DISRUPTION ALERTS
    async def get_all_alerts(self, resolved: bool = False, severity: str = None) -> List[dict]:
        if self.use_mock:
        ...
        try:
            client = get_supabase()
            def sync_query():
                query = client.table("disruption_alerts").select("*").eq("resolved", resolved)
    ...
    async def insert_alert(self, alert: dict) -> dict:
        ...
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("disruption_alerts").insert(alert).execute())
    ...
    async def insert_alerts(self, alerts: List[dict]) -> int:
        ...
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("disruption_alerts").insert(alerts).execute())
            return len(result.data) if result.data else 0
        except Exception as e:
            self._handle_error(e)
    ...
    async def resolve_alert(self, alert_id: str) -> bool:
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(lambda: client.table("disruption_alerts").update(data).eq("id", alert_id).execute())
            return len(result.data) > 0
        except Exception as e:
            self._handle_error(e)
    ...
    async def save_optimization(self, opt: dict) -> dict:
        ...
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("route_optimizations").insert(opt).execute())
            return result.data[0] if result.data else {}
        except Exception as e:
            self._handle_error(e)
    ...
    async def get_optimizations_by_shipment(self, shipment_id: str) -> List[dict]:
        ...
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("route_optimizations").select("*").eq("shipment_id", shipment_id).execute())
            return result.data or []
        except Exception as e:
            self._handle_error(e)
    ...
    async def accept_optimization(self, optimization_id: str) -> bool:
        ...
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("route_optimizations").update({"accepted": True}).eq("id", optimization_id).execute())
            return len(result.data) > 0
        except Exception as e:
            self._handle_error(e)
    ...
    async def save_metrics_snapshot(self, metrics: dict) -> None:
        ...
        try:
            ...
            if 'disruptions_detected_today' in db_metrics:
                db_metrics['disruptions_detected'] = db_metrics.pop('disruptions_detected_today')
            if 'routes_optimized_today' in db_metrics:
                db_metrics['routes_optimized'] = db_metrics.pop('routes_optimized_today')
            
            db_metrics["snapshot_at"] = now_iso()
            await anyio.to_thread.run_sync(lambda: client.table("metrics_snapshots").insert(db_metrics).execute())
        except Exception as e:
            self._handle_error(e)

    async def get_latest_metrics(self) -> Optional[dict]:
        if self.use_mock:
            return self._map_metrics(self._metrics_history[-1].copy()) if self._metrics_history else None
        try:
            client = get_supabase()
            result = await anyio.to_thread.run_sync(lambda: client.table("metrics_snapshots").select("*").order("snapshot_at", desc=True).limit(1).execute())
    ...
    async def get_metrics_history(self, days: int = 7) -> List[dict]:
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(lambda: client.table("metrics_snapshots").select("*").gte("snapshot_at", cutoff).order("snapshot_at", desc=False).execute())
    ...
    # AI LOGS
    async def log_ai_call(self, analysis_type: str, input_summary: str, gemini_response: str, tokens_used: int, latency_ms: int) -> None:
        if self.use_mock:
            return
        try:
            client = get_supabase()
            ...
            await anyio.to_thread.run_sync(lambda: client.table("ai_analysis_logs").insert(data).execute())
        except Exception as e:
